...
def search_songs(message):
    ...
    results = {name: info for name, info in songs.items() if query in name.lower()}
    
    if results:
        markup = types.InlineKeyboardMarkup()
        for name, info in results.items():
            # دکمه شیشه‌ای که وقتی زده شد لینک داخل ربات ارسال شود
...
def inline_query_handler(inline_query):
    ...
    for name, info in songs.items():
        if query_text in name.lower() or query_text == "":
            markup = types.InlineKeyboardMarkup()
            btn = types.InlineKeyboardButton(
